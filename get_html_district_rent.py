# -*- coding: utf-8 -*-
"""
Created on Mon Aug 27 19:07:01 2018
...
def get_big_district_link(url,headers):
    ...
    name_cn=link.xpath('//dd[@data-index="0"]/div[@class="option-list"]/a/text()')    
    href=link.xpath('//dd[@data-index="0"]/div[@class="option-list"]/a/@href')
    district=pd.DataFrame({'name_cn':name_cn[1:],'href':href[1:]})  #第1条为“不限”，去除
...
def get_small_district_link(big_district_link,headers):
    #big_district_link='https://sh.lianjia.com/zufang/pudong/'
    r=requests.get(big_district_link,headers=headers,timeout=30)
    html=r.content
    link=etree.HTML(html,parser=etree.HTMLParser(encoding='utf-8'))
    
    name_cn=link.xpath('//div[@class="option-list sub-option-list"]/a/text()')
    href=link.xpath('//div[@class="option-list sub-option-list"]/a/@href')
    district=pd.DataFrame({'name_cn':name_cn[1:],'href':href[1:]})
    href_split=pd.DataFrame(x.split('/') for x in district['href'])
    district['name_pinyin']=href_split[2]
...
def get_district_link(url,city,headers,save_path='./data_district'):
    ...
    try:
        ...
        df=pd.read_csv('%s/链家租房小区域列表%s.csv'%(save_path,city),engine='python',encoding='utf_8_sig')
        print('查询到“链家租房小区域列表%s.csv”，直接导入...'%city)
        #district_list.head()
    except:
        ##小区域数据来源二：从网站上爬取
        big_district=get_big_district_link(url,headers)
        len_big_district=len(big_district)
        print('未查询到“链家租房小区域列表%s.csv”，共%s个大区域，爬取并保存...'%(city,len_big_district))
        ...
        save_df_to_csv(df,'链家租房小区域列表'+city,save_path)
...
def get_totalpage_num(url,headers):
    ...
    html=r.content
    link=etree.HTML(html,parser=etree.HTMLParser(encoding='utf-8'))
    page=link.xpath('//div[@class="page-box house-lst-page-box"]/@page-data')
    if len(page)==0:
        total_page=0
    else:
...
#保存字符串到txt文件
def save_str_to_txt(file,file_name,save_path='../LianJiaSaveData/save_html_rent/city_date'):    
...
def save_df_mongodb(df,database,table_name,city,datestr):
    # import pymongo
    # import pandas as pd
    # df为dataframe
...
#从数据库database中读取数据
def read_df_mongodb(database,table_name,city,datestr):
    #import pymongo
    #import pandas as pd
...
class Para:
    ...
    url_base_list=['https://%s.lianjia.com/zufang/'%x for x in city_list]
    ...
    for k in range(0,len_city):
        ...
        while True:
            ...
            for i in range(0,len(district_link_list)):
                ...
                table=para.database[para.table_name]
                if table.find_one({'city':city,'datestr':para.datestr,'district':district}):
                    ...
                    try:
                        ...
                        if total_page==0:
                            ...
                            save_df_mongodb(df,para.database,para.table_name,city,para.datestr)
